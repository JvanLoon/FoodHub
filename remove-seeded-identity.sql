...
CREATE
TEMP TABLE seeded_user_ids (id text PRIMARY KEY) ON COMMIT DROP;
INSERT INTO seeded_user_ids (id)
VALUES ('c2f0b2b0-0000-0000-0000-000000000001'), -- admin@foodhub.local
       ('c2f0b2b0-0000-0000-0000-000000000002');
-- user@foodhub.local
...
WHERE "UserId" IN (SELECT id FROM seeded_user_ids);
...
DELETE
FROM "RecipeBlackLists"
WHERE "UserId" IN (SELECT id::uuid FROM seeded_user_ids);
...
DELETE
FROM "AspNetUsers"
WHERE "Id" IN (SELECT id FROM seeded_user_ids);
...
\echo
'Seeded accounts removed. Verify with:'
