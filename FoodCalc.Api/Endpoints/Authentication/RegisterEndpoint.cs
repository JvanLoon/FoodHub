...
public class RegisterEndpoint(UserManager<IdentityUser> userManager) : Endpoint<RegisterDto>
{
	...
	public override async Task HandleAsync(RegisterDto req, CancellationToken ct)
	{
		...
		if (!result.Succeeded)
		{
			...
			await Send.StringAsync(result.Errors.First()
										 .Description, 400, cancellation: ct);
			return;
		}
