...
{
	public async Task<ErrorOr<MealPlanEntryDto>> Handle(AddMealPlanEntryCommand request, CancellationToken cancellationToken)
	{
		try
		{
			...
			var dayCount = await context.MealPlanEntries
				.CountAsync(m => m.UserId == request.UserId && m.Date == request.Date, cancellationToken);
			if (dayCount >= MealPlanConstants.MaxRecipesPerDay)
				return Error.Validation(description: $"A day can hold at most {MealPlanConstants.MaxRecipesPerDay} recipes.");
